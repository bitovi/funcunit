steal.then(function () {
	...
	FuncUnit.jquery(window).load(function () {
		if (document.readyState != readystate) FuncUnit.support.readystate = true;
	});
	...
	var unloadLoader, loadSuccess, firstLoad = true,
		currentDocument, onload = function () {
	...
	FuncUnit.jquery(window).unload(function () {
		if (FuncUnit._window) FuncUnit._window.close();
	})
