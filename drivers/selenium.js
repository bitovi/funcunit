	...
	if (navigator.userAgent.match(/Rhino/) && FuncUnit.browsers !== undefined) {
		...
		(function () {
			...
			QUnit.done = function (failures, total) {
				...
				print("\nALL DONE " + failures + ", " + total + (FuncUnit.showTimestamps ? (' - ' + formattedtime + ' seconds') : ""));
				browser++;
				if (browser < FuncUnit.browsers.length) {
					print("\nSTARTING " + FuncUnit.browsers[browser]);

					FuncUnit.selenium = new DefaultSelenium(FuncUnit.serverHost, FuncUnit.serverPort, FuncUnit.browsers[browser], location);
			...
			FuncUnit.selenium = new DefaultSelenium(FuncUnit.serverHost, FuncUnit.serverPort, FuncUnit.browsers[0], location);
			FuncUnit.starttime = new Date().getTime();
			FuncUnit.selenium.start();
			...
			FuncUnit.confirm = function (answer, callback) {
				...
				FuncUnit.add({
					method: function (success, error) {
						...
						if (answer) FuncUnit.selenium.chooseOkOnNextConfirmation();
						else FuncUnit.selenium.chooseCancelOnNextConfirmation();
			...
			FuncUnit.$ = function (selector, context, method) {
				...
				for (var a = 0; a < args.length; a++) {
					if (a == 1) { //context
						...
						else args[a] = convertToJson(args[a]);
					}
				}
