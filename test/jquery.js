 ...
 */
	jQuery.event = {
		...
		handle: function (event) {
			var all, handlers, namespaces, namespace_sort = [],
				namespace_re, events, args = jQuery.makeArray(arguments);
	...
	function liveHandler(event) {
		var stop, maxLevel, elems = [],
			selectors = [],
			related, match, handleObj, elem, j, i, l, data, close, namespace, events = jQuery.data(this, "events");
		...
		clone: function (events) {
			...
			var ret = this.map(function () {
				if (!jQuery.support.noCloneEvent && !jQuery.isXMLDoc(this)) {
					...
					.replace(/\=([^="'>\s]+\/)>/g, '="$1">').replace(rleadingWhitespace, "")], ownerDocument)[0];
				} else {
		...
		domManip: function (args, table, callback) {
			...
			if (this[0]) {
				...
				if (first) {
					...
					for (var i = 0, l = this.length; i < l; i++) {
						callback.call(
						table ? root(this[i], first) : this[i], i > 0 || results.cacheable || this.length > 1 ? fragment.cloneNode(true) : fragment);
					}
				}
...
/*
		...
		ajax: function (origSettings) {
			...
			var onreadystatechange = xhr.onreadystatechange = function (isTimeout) {
				...
				if (!xhr || xhr.readyState === 0 || isTimeout === "abort") {
					...
					status = isTimeout === "timeout" ? "timeout" : !jQuery.ajax.httpSuccess(xhr) ? "error" : s.ifModified && jQuery.ajax.httpNotModified(xhr, s.url) ? "notmodified" : "success";
	...
	jQuery.each(["Height", "Width"], function (i, name) {
		...
		jQuery.fn[type] = function (size) {
			...
			elem.document.compatMode === "CSS1Compat" && elem.document.documentElement["client" + name] || elem.document.body["client" + name] :
