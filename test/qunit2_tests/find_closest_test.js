...
QUnit.test("closest", function(){
	F("a:contains('Holler')").closest("#foo").click(function(){
		QUnit.assert.ok(this.hasClass("iWasClicked"),"we clicked #foo")
	})
	F("a:contains('Holler')").closest(".baz").click(function(){
		QUnit.assert.ok(this.hasClass("iWasClicked"),"we clicked .baz")
	})
	
})

QUnit.test("find with traversers", function(){
	F(":contains('Holler')")
		.closest("#foo")
		.find(".combo")
		.hasClass("combo", true)
