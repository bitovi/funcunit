...
steal.plugins('funcunit/qunit', 'funcunit/qunit/rhino').then('resources/jquery', 'resources/json', 'resources/selector').plugins('funcunit/syn')
...
.then(function () {
	...
	window.jQuery.extend(FuncUnit, {
		...
		getAbsolutePath: function (path) {
			...
			var fullPath, root = FuncUnit.jmvcRoot || steal.root.path;
			...
			if (/^http/.test(path)) fullPath = path;
			return fullPath;
		},
		/**
		 * @attribute browsers
			...
			if (queue.length > 0) {
				...
				setTimeout(function () {
					...
					function () {
						...
						incallback = true;
						if (next.callback) next.callback.apply(next.bind || null, arguments);
						incallback = false;
			...
			if (isWait) {
				...
				if (typeof tester != 'function') {
					...
					tester = function (val) {
						...
						return QUnit.equiv(val, testVal) || (testVal instanceof RegExp && testVal.test(val));
					}
				}
...
//now add drivers
.then('resources/selenium_start', 'drivers/selenium', 'drivers/standard');